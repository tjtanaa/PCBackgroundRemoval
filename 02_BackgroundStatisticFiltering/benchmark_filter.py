...
def filter_point_cloud_gpu(point_cloud, stats_df, strategy_index):
    ...
    within_range = (range_values >= lower_bounds_gpu) & (range_values <= upper_bounds_gpu)
    ...
    # Free GPU memory
    del point_cloud_gpu, range_values, lower_bounds_gpu, upper_bounds_gpu, valid_stats_gpu, keep_mask_gpu
...
def warm_up_gpu(point_cloud, stats_df, num_warmup=5):
    ...
    for i in range(num_warmup):
        ...
        for strategy_idx in range(1, 5):
            _, _ = filter_point_cloud_gpu(point_cloud, stats_df, strategy_idx)
...
def benchmark_filtering(input_dir, stats_file, output_dir, use_gpu=True, num_frames=10, num_warmup=5):
    """
    ...
    pc_filename_list = [os.path.join(input_dir, filename) for filename in os.listdir(input_dir) if ".bin" in filename]
    ...
    for file_path in tqdm(pc_filename_list[:num_frames]):
        ...
        for strategy_idx, strategy_name in strategies.items():
            ...
            # Filter point cloud
            if use_gpu:
                filtered_point_cloud, removed_count = filter_point_cloud_gpu(
            ...
            filtered_point_cloud.tofile(os.path.join(output_dir, f"frame_{file_path.split(os.sep)[-1]}"))
...
def generate_benchmark_plots(benchmark_df, output_dir):
    ...
    avg_metrics = benchmark_df.groupby("strategy").agg({
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark point cloud filtering based on range statistics")
    parser.add_argument("--input_dir", required=True, help="Directory containing point cloud data files")
    parser.add_argument("--stats_file", required=True, help="Path to the CSV file with point statistics")
    parser.add_argument("--output_dir", required=True, help="Directory to save filtered point clouds and benchmark results")
    parser.add_argument("--cpu", action="store_true", help="Use CPU instead of GPU")
    parser.add_argument("--num_frames", type=int, default=10, help="Number of frames to process for benchmarking")
